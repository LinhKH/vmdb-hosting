			...
			<div class="block-content">
				<div class="block-group-search">
					<div class="search-row">
						...
						<div class="field-search">
							<label for="004" class="label-control"
								>在庫管理</label
							>
							<div class="field-search-input">
								<select
									...
									class="form-control"
									name="item_yn"
									v-model="search.item_yn"
								>
									<option value="">選択</option>
									<option
										v-for="(
											value, key
										) in commonMst.option_item_yn"
										:key="key"
										:value="value.code_value"
									>
										{{ value.disp_value }}
...
export default {
	...
	setup() {
		...
		const optionListSearch = () => {
			...
			ServiceAPI.axiosCall("option_from_list", search.value)
				.then((res) => {
					if (!res.error) {
